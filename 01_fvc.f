      ...
      DOUBLE PRECISION new_width, z1, z2, r
...
c       Giving the initial noise, change the width in params.inc file for different noise      	
      	u1=rand()*new_width-0.5*new_width
	u2=rand()*new_width-0.5*new_width
	theta=acos(rho)
	z1 = sqrt(1.d0 - rho**2)*u1
        z2 = sqrt(1.d0 - rho**2) * (u1 * cos(theta) + u2 * sin(theta))
 ...
 44   continue
      ...
      subroutine generate_correlated_noise(z1, z2, new_width, rho)
      double precision z1, z2, new_width, rho, u1, u2, theta

      ! Generate two independent random numbers between -0.5 and 0.5
      ...
      ! Set correlation coefficient (choose desired value)
      
      ! Compute the correlation angle
      theta = acos(rho)

      ! Calculate correlated noise terms
      z1 = sqrt(1.d0 - rho**2) * u1 * new_width
      z2 = sqrt(1.d0 - rho**2) * (u1 * cos(theta) + u2 * sin(theta)) * new_width
      end subroutine generate_correlated_noise
